...
Config.PoliceLocations = {
    ...
    { coords = vector3(458.47, -1024.77, 28.38), heading = 269.78, }, -- MRPD UNDERGROUND PARKING
    { coords = vector3(-699.82, 357.26, 78.12), heading = 81.99, }, -- PILL BOX GARAGE
}
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
			...
			if GetVehicleLivery(vehicle) == 0 then stockinstall = Loc[Config.Lan]["common"].current icon = "fas fa-check" disabled = true else stockinstall = "" end
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-paint-roller", isMenuHeader = true, header = Loc[Config.Lan]["livery"].menuheader, txt = 'Amount of Options: '..GetVehicleLiveryCount(vehicle) }
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "jim-mechanic:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Police:Apply", args = { id = tostring(0), old = true } } }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
				if GetVehicleLivery(vehicle) == v.id then icon = "fas fa-check" disabled = true end
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = k..". "..v.name, txt = v.install, params = { event = 'jim-mechanic:client:Police:Apply', args = { id = tostring(v.id), old = true } } }
			end
		elseif oldlivery ~= true then
			...
			if GetVehicleMod(vehicle, 48) == -1 then stockinstall = Loc[Config.Lan]["common"].current icon = "fas fa-check" disabled = true else stockinstall = "" end
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-paint-roller", isMenuHeader = true, header = Loc[Config.Lan]["livery"].menuheader, txt = 'Amount of Options: '..GetNumVehicleMods(vehicle, 48)+1 }
				LiveryMenu[#LiveryMenu + 1] = { icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "jim-mechanic:client:Police:Menu" }, }
				LiveryMenu[#LiveryMenu + 1] = { icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Police:Apply", args = { id = tostring(-1) } } }
			for k, v in pairs(validMods) do
				local icon = "" local disabled = false
...
RegisterNetEvent('jim-mechanic:client:Police:Plates', function()
	...
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle)
		if DoesEntityExist(vehicle) then
			...
			{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "jim-mechanic:client:Police:Menu" } } }
...
RegisterNetEvent('jim-mechanic:client:Police:Spoiler', function()
	...
	if DoesEntityExist(vehicle) then
		...
		local spoilerMenu = {
				{ icon = "fas fa-car-burst", isMenuHeader = true, header = Loc[Config.Lan]["spoilers"].menuheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1,	},
				{ icon = "fas fa-circle-arrow-left", header = "", txt = string.gsub(Loc[Config.Lan]["common"].ret, "⬅️ ", ""), params = { event = "jim-mechanic:client:Police:Menu" } },
				{ icon = icon, isMenuHeader = disabled, header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "jim-mechanic:client:Police:Spoilers:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				local icon = "" local disabled = false
